...
def trainProcess(trainBar): 
    net.train(True)
    for batch_idx, (trainImgs, labels) in enumerate(trainBar):
        trainImgs, labels = trainImgs.to(device), labels.to(device)  # 将图像和标签数据移动到指定的设备上（如GPU）
        net.zero_grad()  # 初始化梯度
        ...
        trainBar.set_description("[%d/%d] Loss: %.4f, Acc: %.4f" % (epoch, EPOCHS, loss.item(), accuracy.item()))  # 设置进度条显示当前epoch、损失和准确率

def testProcess(testBar):
    net.train(False)
    total_loss = 0
    correct_num = 0
    with torch.no_grad():
        for batch_idx, (testImgs, labels) in enumerate(testBar):
            ...
            total_loss += loss  # 累加计算损失
            correct_num += torch.sum(predictions == labels)  # 累加计算正确预测数量
            ave_testAccuracy = correct_num / (BATCH_SIZE * len(testDataLoader))
            ave_testLoss = total_loss / len(testDataLoader)
            testBar.set_description("[%d/%d] Ave Test Loss: %.4f, Ave Test Acc: %.4f" % (epoch, EPOCHS, ave_testLoss.item(), ave_testAccuracy.item()))
    history['Test Loss'].append(ave_testLoss.item())
    history['Test Accuracy'].append(ave_testAccuracy.item())
...
for epoch in range(1, EPOCHS + 1):
    trainBar = tqdm(trainDataLoader, unit = 'step')
    trainProcess(trainBar)
    trainBar.close()
    
    testBar = tqdm(testDataLoader, unit = 'step')
    testProcess(testBar)
    testBar.close()
